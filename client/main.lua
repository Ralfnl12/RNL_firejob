...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('RNL_firejob:getOtherPlayerData', function(data)
		local elements = {
	...
	ESX.TriggerServerCallback('RNL_firejob:getOtherPlayerData', function(data)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if data.current.value then
				TriggerServerEvent('RNL_firejob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function LookupVehicle(elementF)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('RNL_firejob:getVehicleInfos', function(retrivedInfo)
				local elements = {
					{unselectable = true, icon = "fas fa-car", title = element.title},
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('RNL_firejob:getOtherPlayerData', function(playerData)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			TriggerServerEvent('RNL_firejob:message', GetPlayerServerId(player), TranslateCap('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('RNL_firejob:getArmoryWeapons', function(weapons)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local data = {current = element}
			ESX.TriggerServerCallback('RNL_firejob:removeArmoryWeapon', function()
				ESX.CloseContext()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.OpenContext("right", elements, function(menu,element)
		local data = {current = element}
		ESX.TriggerServerCallback('RNL_firejob:addArmoryWeapon', function()
			ESX.CloseContext()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('RNL_firejob:buyWeapon', function(bought)
				if bought then
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('RNL_firejob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('RNL_firejob:getPlayerInventory', function(inventory)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('RNL_firejob:putStockItems', itemName, count)
...
AddEventHandler('RNL_firejob:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('RNL_firejob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('RNL_firejob:hasExitedEntityZone', function(entity)
...
AddEventHandler('RNL_firejob:handcuff', function()
...
RegisterNetEvent('RNL_firejob:unrestrain')
AddEventHandler('RNL_firejob:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
AddEventHandler('RNL_firejob:drag', function(copId)
	if isHandcuffed then
...
AddEventHandler('RNL_firejob:putInVehicle', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
...
AddEventHandler('RNL_firejob:OutVehicle', function()
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('RNL_firejob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('RNL_firejob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
		...
		local Sleep = 1500
			...
			if closestDistance ~= -1 and closestDistance <= 3.0 then
				if LastEntity ~= closestEntity then
					TriggerEvent('RNL_firejob:hasEnteredEntityZone', closestEntity)
					LastEntity = closestEntity
				end
			else
				if LastEntity then
					TriggerEvent('RNL_firejob:hasExitedEntityZone', LastEntity)
					LastEntity = nil
				end
			end
		Wait(Sleep)
...
RegisterNetEvent('RNL_firejob:updateBlip')
AddEventHandler('RNL_firejob:updateBlip', function()
...
AddEventHandler('esx:onPlayerSpawn', function(spawn)
	...
	if not hasAlreadyJoined then
		TriggerServerEvent('RNL_firejob:spawned')
	end
